...
import { useBudgetExecutionContext, useCreateMouvementFinancier } from "../hooks/useComptableBudget";
...
export function ComptableBudgetDetailPage() {
  ...
  const [formOpen, setFormOpen] = useState<false | TypeMouvementFinancier>(false);
  ...
  function openForm(type: TypeMouvementFinancier) {
    setForm({ ...emptyForm, type_mouvement: type, date_mouvement: new Date().toISOString().slice(0, 10) });
    setFormOpen(type);
  }
  ...
  function validateMovement() {
    ...
    if (form.type_mouvement === "sortie" && !form.ligne_budgetaire_id) {
  ...
  function submitMovement() {
    ...
    const descriptionParts = [form.beneficiaire ? `Beneficiaire: ${form.beneficiaire}` : "", form.description].filter(Boolean);
    const payload: MouvementFinancierCreate = {
      projet_id: budget.projet_id,
      budget_id: budget.id,
      ligne_budgetaire_id: form.type_mouvement === "sortie" ? Number(form.ligne_budgetaire_id) : null,
    ...
    createMouvement.mutate(payload, {
      onSuccess: () => {
        setFormOpen(false);
        dispatch(showToast({ message: "Mouvement financier enregistre.", type: "success" }));
      },
  ...
  if (contextQuery.isError || !budget) {
    return <AccessMessage message={contextQuery.isError ? getApiErrorMessage(contextQuery.error) : "Budget introuvable."} title="Erreur" />;
  }
  ...
  return (
    <main className="min-h-screen bg-[#F4F7FA] lg:flex">
        ...
        <div className="mx-auto grid max-w-7xl gap-6">
          ...
          <header className="rounded-lg bg-white p-6 text-left shadow-sm ring-1 ring-[#E5E7EB]">
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
              ...
              <span className={`rounded-full px-3 py-1 text-xs font-semibold ring-1 ${canRecord ? "bg-[#DBEAFE] text-[#2563EB] ring-blue-200" : "bg-[#FEF3C7] text-[#D97706] ring-[#FDE68A]"}`}>
          ...
          {!canRecord ? (
            <div className="rounded-lg border border-[#FDE68A] bg-[#FEF3C7] px-4 py-3 text-sm font-medium text-[#92400E]">
              Ce budget est approuve mais pas encore en execution. Le Comptable ne peut pas enregistrer de mouvements.
            </div>
          ) : null}
          ...
          <section className="grid gap-3 rounded-lg bg-white p-6 text-left shadow-sm ring-1 ring-[#E5E7EB] md:grid-cols-3">
            <div>
              ...
              <p className="mt-2 text-lg font-bold text-[#1F2937]">{formatAmount(execution?.ecart_recettes ?? 0)}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-[#6B7280]">Ecart depenses</p>
              <p className="mt-2 text-lg font-bold text-[#1F2937]">{formatAmount(execution?.ecart_depenses ?? 0)}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-[#6B7280]">Taux execution depenses</p>
              <p className="mt-2 text-lg font-bold text-[#1F2937]">{Number(execution?.taux_execution_depenses ?? 0).toFixed(2)}%</p>
            </div>
          </section>

          <section className="flex flex-wrap justify-end gap-3 rounded-lg bg-white p-6 shadow-sm ring-1 ring-[#E5E7EB]">
            <button className="btn-primary rounded-md px-4 py-2 text-sm font-semibold text-white hover:bg-[#166F48] disabled:opacity-60" disabled={!canRecord} onClick={() => openForm("entree")}>
              Enregistrer une entree
            </button>
            <button className="btn-danger rounded-md px-4 py-2 text-sm font-semibold text-white hover:bg-[#B91C1C] disabled:opacity-60" disabled={!canRecord} onClick={() => openForm("sortie")}>
              Enregistrer une sortie
            </button>
          </section>

          <section className="rounded-lg bg-white p-6 text-left shadow-sm ring-1 ring-[#E5E7EB]">
            <h2 className="text-lg font-bold text-[#1F2937]">Lignes budgetaires prevues</h2>
            <div className="mt-4 overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {lignes.map((ligne) => {
                    const executionLine = execution?.lignes_budgetaires.find((item) => item.ligne_budgetaire_id === ligne.id);
                    return (
                      <tr key={ligne.id} className="border-b border-[#E5E7EB]">
                        ...
                        <td className="px-4 py-3">{formatAmount(executionLine?.montant_realise ?? ligne.montant_realise ?? 0)}</td>
                        <td className="px-4 py-3">{formatAmount(executionLine?.ecart_montant ?? ligne.ecart_montant ?? 0)}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <section className="rounded-lg bg-white p-6 text-left shadow-sm ring-1 ring-[#E5E7EB]">
            <h2 className="text-lg font-bold text-[#1F2937]">Mouvements financiers</h2>
            <div className="mt-4 overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-[#F9FAFB]">
                  <tr>
                ...
                <tbody>
                  {mouvements.length === 0 ? (
                    <tr>
                      <td className="px-4 py-8 text-center text-[#6B7280]" colSpan={5}>Aucun mouvement enregistre.</td>
                    </tr>
                  ) : (
      ...
      <PopupModal open={Boolean(formOpen)} title={formOpen === "sortie" ? "Enregistrer une sortie" : "Enregistrer une entree"} onClose={() => setFormOpen(false)}>
        <div className="grid gap-4 text-left">
          <div className="rounded-md bg-[#F9FAFB] px-3 py-2 text-sm text-[#6B7280]">
            Projet: <span className="font-semibold text-[#1F2937]">{budget.projet?.titre ?? budget.projet_id}</span> - Budget: <span className="font-semibold text-[#1F2937]">{budget.reference}</span>
          </div>
          {formOpen === "sortie" ? (
            <label className="grid gap-1 text-sm font-semibold text-[#374151]">
              Ligne budgetaire concernee
              <select className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" value={form.ligne_budgetaire_id} onChange={(event) => updateForm("ligne_budgetaire_id", event.target.value)}>
                <option value="">Choisir une ligne de depense</option>
                {depenseLines.map((ligne) => (
                  <option key={ligne.id} value={ligne.id}>{ligne.libelle} - {formatAmount(ligne.montant_prevu)}</option>
                ))}
              </select>
            </label>
          ) : null}
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="grid gap-1 text-sm font-semibold text-[#374151]">
              Libelle
              <input className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" value={form.libelle} onChange={(event) => updateForm("libelle", event.target.value)} />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-[#374151]">
              Categorie
              <input className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" value={form.categorie} onChange={(event) => updateForm("categorie", event.target.value)} />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-[#374151]">
              Montant
              <input className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" min="0.01" step="0.01" type="number" value={form.montant} onChange={(event) => updateForm("montant", event.target.value)} />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-[#374151]">
              Date
              <input className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" max={budget.projet?.date_fin_prevue ?? undefined} min={budget.projet?.date_debut_prevue ?? undefined} type="date" value={form.date_mouvement} onChange={(event) => updateForm("date_mouvement", event.target.value)} />
            </label>
            {formOpen === "sortie" ? (
              <label className="grid gap-1 text-sm font-semibold text-[#374151]">
                Beneficiaire
                <input className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" value={form.beneficiaire} onChange={(event) => updateForm("beneficiaire", event.target.value)} />
              </label>
            ) : null}
            <label className="grid gap-1 text-sm font-semibold text-[#374151]">
              Mode de paiement
              <input className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" value={form.mode_paiement} onChange={(event) => updateForm("mode_paiement", event.target.value)} />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-[#374151]">
              Reference
              <input className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" value={form.reference_paiement} onChange={(event) => updateForm("reference_paiement", event.target.value)} />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-[#374151]">
              Piece justificative
              <input className="rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" value={form.piece_justificative} onChange={(event) => updateForm("piece_justificative", event.target.value)} />
            </label>
          </div>
          <label className="grid gap-1 text-sm font-semibold text-[#374151]">
            Description
            <textarea className="min-h-24 rounded-md border border-[#E5E7EB] px-3 py-2 font-normal" value={form.description} onChange={(event) => updateForm("description", event.target.value)} />
          </label>
