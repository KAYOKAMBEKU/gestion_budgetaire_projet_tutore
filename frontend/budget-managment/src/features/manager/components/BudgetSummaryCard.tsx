...
export function BudgetSummaryCard({ currency, lines }: { currency?: CurrencyCode; lines: DraftBudgetLine[] }) {
  ...
  const activitiesCount = new Set(lines.map((line) => line.activite).filter(Boolean)).size;
    ...
    <section className="rounded-lg border border-[#E5E7EB] bg-white p-5 text-left shadow-sm">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-lg bg-[#F9FAFB] p-4">
          ...
          <p className="mt-1 text-xs font-semibold text-[#6B7280]">{activitiesCount} activite(s)</p>
        </div>
        <div className="rounded-lg bg-[#DCFCE7] p-4">
          ...
          <p className="mt-2 text-lg font-bold text-[#15803D]">{formatAmount(totalRecettes, currency)}</p>
        </div>
        <div className="rounded-lg bg-[#FEE2E2] p-4">
          ...
          <p className="mt-2 text-lg font-bold text-[#B91C1C]">{formatAmount(totalDepenses, currency)}</p>
        </div>
        <div className="rounded-lg bg-[#DBEAFE] p-4">
          ...
          <p className="mt-2 text-lg font-bold text-[#1D4ED8]">{formatAmount(solde, currency)}</p>
        </div>
      </div>
    </section>
  );
